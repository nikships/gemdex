...
- **Breaking: the npx package (`gemdex-mcp`) and the macOS app are local-only and run without Gemini.**
  ...
  - **Upgrading from a Gemini-based release:** existing memories stay readable, editable, and exportable. Run `npx gemdex-mcp migrate` (or use Settings in the app) to re-embed them locally. Recall and hygiene stay unavailable until the migration finishes.
  - **Apple Silicon is required.** Every tool, including list, get, and export, needs the local model, so on Intel Macs, Linux, and Windows this release cannot open an existing `~/.gemdex` store. On those machines, pin `gemdex-mcp@1.0.47` in your MCP config (not `@latest`), or export your memories with it before upgrading.
  - **Remote mode is removed from the npx package and the app.** This drops `GEMDEX_MODE`, `init-remote`, `remote`, `mode`, `import-local-to-remote`, `sync-history`, and the app's remote settings. To reach a self-hosted pool, connect an agent to the Streamable HTTP MCP endpoint (`gemdex-mcp-http`).
  - **Local attachments are limited to text files** (`.txt`, `.json`, `.jsonl`, `.ndjson`). They are stored with the memory and read back with `read_attachment`. Image, audio, video, and PDF attachments are rejected, but existing media on a memory survives updates and imports.
  - The self-hosted stack (`gemdex-server`, `gemdex-mcp-http`, `gemdex-web`, `deploy/`) is unchanged and keeps server-side Gemini embedding and session digestion. The BYOI integration harness moved to `packages/server/integration/byoi.mjs`.
