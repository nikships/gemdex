...
import { Context, FileSynchronizer, envManager } from "gemdex-core";
...
const SYNC_LOCK_STALE_ENV = "GEMDEX_SYNC_LOCK_STALE_MS";

function isBackgroundSyncEnabled(): boolean {
    const value = envManager.get("GEMDEX_BACKGROUND_SYNC");
    if (!value) {
        return true;
    }
    ...
    switch (value.trim().toLowerCase()) {
        ...
        default:
            console.warn(
                `[SYNC-DEBUG] Invalid GEMDEX_BACKGROUND_SYNC value '${value}'. ` +
...
function getBackgroundSyncIntervalMs(): number {
    const value = envManager.get("GEMDEX_SYNC_INTERVAL_MS");
    if (!value) {
    ...
    if (!Number.isFinite(intervalMs) || intervalMs < MIN_SYNC_INTERVAL_MS) {
        console.warn(
            `[SYNC-DEBUG] Invalid GEMDEX_SYNC_INTERVAL_MS value '${value}'. ` +
...
export class SyncManager {
    ...
    private getSyncLockPath(): string {
        return path.join(os.homedir(), ".gemdex", "mcp-sync.lock");
    }
    ...
    public startBackgroundSync(): void {
        ...
        if (!isBackgroundSyncEnabled()) {
            console.log('[SYNC-DEBUG] Background sync is disabled via GEMDEX_BACKGROUND_SYNC=false.');
            return;
        }
    ...
    /**
     * Read GEMDEX_TRIGGER_WATCHER. Default ON — the watcher is cheap and only
    ...
    private isTriggerWatcherEnabled(): boolean {
        const v = (envManager.get('GEMDEX_TRIGGER_WATCHER') ?? '').trim().toLowerCase();
        if (!v) return true;
        if (['1', 'true', 'yes', 'on'].includes(v)) return true;
        if (['0', 'false', 'no', 'off'].includes(v)) return false;
        console.warn(
            `[SYNC-DEBUG] Invalid GEMDEX_TRIGGER_WATCHER value '${v}'. ` +
    ...
    private setupTriggerWatcher(): void {
        if (!this.isTriggerWatcherEnabled()) {
            console.log('[SYNC-DEBUG] Trigger watcher disabled via GEMDEX_TRIGGER_WATCHER');
            return;
        }
        ...
        const contextDir = path.join(os.homedir(), '.gemdex');
