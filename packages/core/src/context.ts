...
export class Context {
    ...
    constructor(config: ContextConfig = {}) {
        ...
        if (!config.embedding && !geminiKey) {
            throw new Error('GEMINI_API_KEY is required when no embedding instance is supplied');
        }
        this.embedding = config.embedding || new GeminiEmbedding({
            apiKey: geminiKey!,
            ...(envManager.get('EMBEDDING_MODEL') && { model: envManager.get('EMBEDDING_MODEL')! }),
 ...
 */
    ...
    private async loadIgnorePatterns(codebasePath: string, additionalIgnorePatterns: string[] = []): Promise<string[]> {
        try {
            ...
            // Load global ~/.gemdex/.gemdexignore
    ...
    /**
     * Load global ignore file from ~/.gemdex/.gemdexignore
     * @returns Array of ignore patterns
     */
    private async loadGlobalIgnoreFile(): Promise<string[]> {
        try {
            const homeDir = require('os').homedir();
            const globalIgnorePath = path.join(homeDir, '.gemdex', '.gemdexignore');
            return await this.loadIgnoreFile(globalIgnorePath, 'global .gemdexignore');
        } catch (error) {
