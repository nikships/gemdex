...
test('blob store defaults to file with optional BLOB_DIR', () => {
    const cfg = loadServerConfig({
        env: { GEMDEX_SERVER_TOKEN: 'secret', BLOB_STORE: 'file', BLOB_DIR: '/var/lib/gemdex/blobs' },
        argv: [],
    });
...
test('BLOB_STORE=s3 requires a bucket', () => {
    assert.throws(
        () => loadServerConfig({ env: { GEMDEX_SERVER_TOKEN: 'secret', BLOB_STORE: 's3' }, argv: [] }),
        /S3_BUCKET/,
    );
});
...
test('config file can provide blobStore settings', () => {
    ...
    try {
        ...
        const cfg = loadServerConfig({ env: { GEMDEX_SERVER_TOKEN: 'secret', GEMDEX_SERVER_CONFIG: configPath }, argv: [] });
        assert.deepEqual(cfg.blobStore, {
            kind: 's3',
            bucket: 'from-file',
            endpoint: 'http://localhost:9000',
