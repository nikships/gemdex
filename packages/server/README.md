...
(curl, server-side HTTP clients, and reverse proxies) bypass the CORS origin
check but still require a valid bearer token unless `unsafeDevNoAuth` is enabled.
Browser requests only receive `Access-Control-Allow-Origin` when their origin
appears in `GEMDEX_SERVER_ALLOWED_ORIGINS` or `allowedOrigins`. Preflight from
any other origin returns 403. Include exact origins such as:
