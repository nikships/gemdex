...
// codebase. Used by the Droid PostToolUse hook.
//
// Droid passes the hook payload as JSON on stdin. We extract `cwd` and
// write it into the trigger file as a single line. The MCP watcher reads
// that line and runs `reindexByChange` for the matching indexed codebase
// only. An empty file is the legacy "no workspace info" signal and the
// watcher falls back to a sync across every indexed codebase.
